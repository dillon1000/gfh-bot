...
const monthNamePattern =
	/^(?<month>[a-z]+)\s+(?<day>\d{1,2})(?:st|nd|rd|th)?(?:,\s*|\s+)(?<year>\d{4})(?:\s+(?<time>\d{1,2}(?::\d{2})?\s*(?:am|pm)?))?(?:\s+(?<timezone>[a-z]{2,5}|z|[+-]\d{2}(?::?\d{2})?))?$/i;
const isoLikePattern =
	/^(?<year>\d{4})-(?<month>\d{1,2})-(?<day>\d{1,2})(?:[ t](?<time>\d{1,2}(?::\d{2})?\s*(?:am|pm)?))?(?:\s+(?<timezone>[a-z]{2,5}|z|[+-]\d{2}(?::?\d{2})?))?$/i;
const timePattern =
...
const parseClock = (
	...
	if (!match?.groups?.hour) {
		throw new Error(
			`Could not parse market close time. ${getAbsoluteCloseHelp()}`,
		);
	}
...
const zonedDateTimeToUtc = (
	...
	if (
		...
		throw new Error(
			`Could not parse market close time in ${timeZone}. ${getAbsoluteCloseHelp()}`,
		);
	}
...
const parseAbsoluteDateTime = (
	value: string,
): ParsedAbsoluteDateTime | null => {
	const trimmed = value.trim().replace(/\s+/g, " ");
	const monthMatch = monthNamePattern.exec(trimmed);
	if (
		monthMatch?.groups?.month &&
		monthMatch.groups.day &&
...
const assertWithinAbsoluteMarketCloseWindow = (date: Date, now: Date): void => {
	...
	if (deltaMs < marketMinDurationMs) {
		throw new Error(
			"Market close time must be at least 5 minutes in the future.",
		);
	}
};
...
export const parseMarketCloseAt = (value: string, now = new Date()): Date => {
	...
	const timezoneOffset = parsed.timezone
		? (timezoneAbbreviationOffsets.get(parsed.timezone) ??
			parseOffsetTimezone(parsed.timezone))
		: null;
	const closeAt =
		timezoneOffset !== null
