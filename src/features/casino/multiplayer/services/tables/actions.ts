...
const performCasinoTableActionInternal = async (
	...
	withTableLock(input.tableId, async () =>
		runSerializableTransaction(async (tx) => {
			...
			if (
				!options.allowExpiredAction &&
				summary.actionDeadlineAt &&
				summary.actionDeadlineAt.getTime() <= Date.now()
			) {
			...
			if (state.kind === "multiplayer-blackjack") {
				...
				if (input.action === "blackjack_hit") {
					...
					const account = await ensureEconomyAccountTx(
						tx,
						table.guildId,
						input.userId,
					);
				...
				const nextSeatIndex = resolveNextBlackjackActor(
					players,
					state.actingSeatIndex ?? current.seatIndex,
				);
				const nextPlayers: MultiplayerBlackjackPlayerState[] = players.map(
					(player) => {
						...
						if (
							nextSeatIndex !== null &&
							player.seatIndex === nextSeatIndex &&
				...
				const deadline =
					nextSeatIndex === null
						? null
						: setActionDeadline(table.actionTimeoutSeconds);
				...
				if (input.action === "holdem_fold") {
					...
					current.committedThisRound = formatRoundMoney(
						current.committedThisRound + paid,
					);
					current.totalCommitted = formatRoundMoney(
						current.totalCommitted + paid,
					);
					current.actedThisRound = true;
					...
					const raiseDelta = formatRoundMoney(
						normalizedTarget - current.committedThisRound,
					);
					if (raiseDelta > current.stack) {
						throw new Error("You do not have enough chips for that raise.");
					}
					const raiseSize = formatRoundMoney(
					...
					current.totalCommitted = formatRoundMoney(
						current.totalCommitted + raiseDelta,
					);
					current.actedThisRound = true;
					...
					if (raiseSize >= state.minRaise) {
						state.minRaise = formatRoundMoney(
							normalizedTarget - previousCurrentBet,
						);
					}
				} else {
				...
				const pot = formatRoundMoney(
					players.reduce((sum, player) => sum + player.totalCommitted, 0),
				);
				...
				nextState = maybeAdvanceHoldemStreet(
					nextState,
					table.bigBlind ?? defaultHoldemBigBlind,
				);
				if (nextState.street !== "complete") {
					const activePlayers = nextState.players.filter(
						(player) => !player.folded && !player.allIn,
					);
					const nextActor =
						nextState.street !== state.street
...
export const performCasinoTableAction = async (
	input: TableActionInput,
): Promise<CasinoTableSummary> => performCasinoTableActionInternal(input);
