...
const formatSeatName = (table: CasinoTableSummary, seat: CasinoTableSeatSummary): string => {
  if (seat.isBot) {
...
const buildSeatVisual = (
  ...
  const acting = state.completedAt === null && state.actingSeatIndex === seat.seatIndex;
  const payout = player.payout ?? 0;
  const status = payout > 0
...
const buildCenterCopy = (table: CasinoTableSummary, state: MultiplayerHoldemState | null): string[] => {
  if (!state) {
  ...
  const actingSeat = actingPlayer
    ? table.seats.find((seat) => seat.seatIndex === actingPlayer.seatIndex) ?? null
    : null;
...
const drawImageContain = (
  ...
  context.drawImage(image as never, offsetX, offsetY, drawWidth, drawHeight);
};
...
const drawSeatPanel = (
  ...
  context.stroke();
  context.drawImage(visual.acting ? playerTimeIcon : playerBaseIcon, boxX + 12, boxY + 10, 34, 34);
  context.font = '700 19px sans-serif';
  drawLabel(context, truncateToWidth(context, visual.title, 112), boxX + 52, boxY + 31, {
    font: '700 19px sans-serif',
    color: text,
  });
...
const buildBlackjackSeatVisual = (
  ...
  const status = player.outcome
    ...
    : acting
      ? 'To act'
      : player.status;
  const accent = player.outcome === 'player_win' || player.outcome === 'blackjack'
    ? success
...
const drawBlackjackHand = (
  ...
  const totalWidth = cards.length === 0 ? 0 : cardWidth + ((cards.length - 1) * (cardWidth - overlap));
...
const drawBlackjackSeatPanel = (
  ...
  if (state) {
    ...
    if (player) {
      const handPosition = getBlackjackSeatHandPosition(seatIndex, position, boxY);
      const cardWidth = seatIndex === 0 ? 30 : 36;
      const cardHeight = seatIndex === 0 ? 44 : 52;
      drawBlackjackHand(context, player.cards, cardImages, handPosition.centerX, handPosition.y, cardWidth, cardHeight);
    }
  }
...
const buildBlackjackCenterCopy = (
  ...
  if (!state) {
    ...
    return [
      'Blackjack lobby',
      seatedCount >= table.minSeats ? 'Ready to deal' : 'Waiting for more players',
      `Base wager ${formatMoney(table.baseWager ?? 0)}`,
    ];
  }

  const actingSeat = state.actingSeatIndex === null
    ? null
    : table.seats.find((seat) => seat.seatIndex === state.actingSeatIndex) ?? null;
  return [
    state.completedAt ? 'Round complete' : 'Blackjack round in progress',
    actingSeat ? `${formatSeatName(table, actingSeat)} to act` : 'Dealer resolving',
    state.actionDeadlineAt
      ? `${Math.max(0, Math.ceil((new Date(state.actionDeadlineAt).getTime() - Date.now()) / 1000))} seconds left`
      : 'Action clock paused',
  ];
...
export const buildHoldemTableDiagram = async (table: CasinoTableSummary): Promise<DiagramPayload> => {
  ...
  communityCardImages.forEach((cardImage, index) => {
    drawImageContain(context, cardImage, 368 + (index * 96), 276 + centerShiftY, 88, 122);
  });
  ...
  drawLabel(context, `Blinds ${formatMoney(table.smallBlind ?? 0)} / ${formatMoney(table.bigBlind ?? 0)}`, 600, 430 + centerShiftY, {
    font: '700 20px sans-serif',
    color: text,
    align: 'center',
  });
  drawLabel(context, `Buy-in ${formatMoney(table.defaultBuyIn ?? 0)}`, 600, 454 + centerShiftY, {
  ...
  for (let seatIndex = 0; seatIndex < table.maxSeats; seatIndex += 1) {
    drawSeatPanel(context, table, state, seatIndex, playerBaseIcon, playerTimeIcon);
  }
...
export const buildBlackjackTableDiagram = async (table: CasinoTableSummary): Promise<DiagramPayload> => {
  const state = table.state?.kind === 'multiplayer-blackjack' ? table.state : null;
  const canvas = createCanvas(width, height);
  const context = canvas.getContext('2d');
  ...
  drawLabel(context, `Base wager ${formatMoney(table.baseWager ?? 0)}`, 680, 512, {
    font: '700 16px sans-serif',
    color: text,
    align: 'center',
  });
  ...
  if (state) {
    drawLabel(context, state.completedAt === null ? 'Dealer hole card hidden until players finish' : 'Dealer hand revealed', 600, 458, {
      font: '700 17px sans-serif',
      color: muted,
      align: 'center',
    });
  ...
  for (let seatIndex = 0; seatIndex < table.maxSeats; seatIndex += 1) {
    drawBlackjackSeatPanel(context, table, state, seatIndex, playerBaseIcon, playerTimeIcon, cardImages);
  }
