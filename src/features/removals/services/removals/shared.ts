...
export const assertConfiguredMemberRole = (config: RemovalEligibilityConfig): string => {
...
export const getRequestAuthorId = (request: RemovalVoteRequestWithSupports): string =>
  request.supports.find((support) => support.kind === 'request')?.supporterId
  ?? request.supports[0]?.supporterId
  ?? request.targetUserId;

export const buildRemovalPollDescription = (request: RemovalVoteRequestWithSupports): string =>
  [
...
export const recordAutoStartFailure = async (requestId: string, error: unknown): Promise<void> => {
