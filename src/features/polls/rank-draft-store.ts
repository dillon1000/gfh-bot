...
const getDraftKey = (pollId: string, userId: string): string => `poll-rank-draft:${pollId}:${userId}`;
...
export const savePollRankDraft = async (
  ...
  await redis.set(getDraftKey(pollId, userId), JSON.stringify(ranking), 'EX', ttlSeconds);
};
