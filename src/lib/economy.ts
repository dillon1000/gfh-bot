...
// Casino-enabled guilds use a lower daily top-up floor.
const getDailyTopUpFloorForConfig = (
	config: Pick<GuildConfig, "casinoEnabled">,
): number =>
	config.casinoEnabled ? casinoDailyTopUpFloor : defaultDailyTopUpFloor;
...
export const ensureEconomyAccountTx = async (
	...
	const lastTopUpDay = account.lastTopUpAt
		? startOfUtcDay(account.lastTopUpAt).getTime()
		: null;
...
const getEffectiveAccount = (
	account: Pick<
		MarketAccount,
		"bankroll" | "realizedProfit" | "lastTopUpAt"
	> | null,
...
export const getEffectiveEconomyAccountPreview = async (
	...
	return getEffectiveAccount(
		account,
		getDailyTopUpFloorForConfig({
			casinoEnabled: config?.casinoEnabled ?? false,
		}),
		now,
	);
};
...
export const grantEconomyBankroll = async (input: {
	...
	if (input.amount > maximumGrantAmount) {
		throw new Error(
			`Grant amount cannot exceed ${maximumGrantAmount.toFixed(2)} points.`,
		);
	}
...
export const getDailyTopUpFloorForGuild = async (
	guildId: string,
): Promise<number> => {
	...
	return getDailyTopUpFloorForConfig({
		casinoEnabled: config?.casinoEnabled ?? false,
	});
};
